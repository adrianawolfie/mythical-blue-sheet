(() => {
  ...
  async function load() {
    const response = await window.apiFetch("/api/admin/campaigns", { headers: { Accept: "application/json" } });
  ...
  document.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (add) {
        ...
        if (!userId) return;
        const response = await window.apiFetch(`/api/admin/campaigns/${encodeURIComponent(campaignId)}/players`, {
          method: "POST",
      ...
      if (remove) {
        ...
        const response = await window.apiFetch(`/api/admin/campaigns/${encodeURIComponent(campaignId)}/players/${encodeURIComponent(userId)}`, { method: "DELETE" });
      ...
      if (assignDM) {
        ...
        const userId = select ? select.value : "";
        const response = await window.apiFetch(`/api/admin/campaigns/${encodeURIComponent(campaignId)}/dm`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId })
        });
