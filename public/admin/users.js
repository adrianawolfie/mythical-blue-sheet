(() => {
  ...
  async function load() {
    const response = await window.apiFetch("/api/admin/users", { headers: { Accept: "application/json" } });
    ...
    data.Users.forEach((user) => {
      ...
      status.addEventListener("click", async () => {
        const response = await window.apiFetch(`/api/admin/users/${encodeURIComponent(user.ID)}`, {
          method: "PUT",
  ...
  form.addEventListener("submit", async (event) => {
    ...
    error.textContent = "";
    const response = await window.apiFetch(`/api/admin/users/${encodeURIComponent(editingUserId)}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
